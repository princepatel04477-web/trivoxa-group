...
export function CardGrid({ cols = 3, items }: { cols?: 2 | 3 | 4; items: CardItem[] }) {
  return (
    <div className={`tvx-grid tvx-grid--${cols}`}>
      {items.map((c, i) => {
        const inner = (
          <>
            {c.icon && <div className="tvx-card__icon">{c.icon}</div>}
            <h3>{c.title}</h3>
            <p>{c.description}</p>
            {c.cta && <span className="tvx-card__link">{c.cta}</span>}
          </>
        );
        return c.href ? (
          <Link key={i} href={c.href} className="tvx-card">
            {inner}
          </Link>
        ) : (
          <div key={i} className="tvx-card">
            {inner}
          </div>
        );
      })}
